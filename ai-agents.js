...
async function callRootLessonGenerator(entry, learnedTranslits = []) {
  ...
  const prompt = `
    You are a Quranic linguistics teacher. Teach ONE lesson the way a good
    Arabic grammarian teaches: start from the root, show how the patterns build
    words out of it, and explain WHY each derived word means what it means —
    never just assert that it does.
    ...
    HOW TO TEACH — this matters as much as the content:
    - Write in warm, direct second person. Assume an intelligent adult who
      knows no Arabic grammar yet, and never talks down to.
    - Always answer "why", not only "what". If a pattern produces an unexpected
      meaning, that IS the lesson — explain the mechanism.
    - Name technical terms in Arabic WITH the English, e.g. "متعدٍّ (transitive)",
      "الأجوف (hollow verb)", "الإعلال (weak-letter substitution)". Define each
      one the moment you use it.
    - Write ALL Arabic fully vowelled (with harakat) — قَرَأَ, never قرأ.
    - Give a transliteration for every Arabic word.
${isParticles ? '' : `
    PRODUCE:

    1. "rootMeaning": what the three letters ${entry.root} carry in common —
       the thread running through every word built from them, in 1-2 sentences.

    2. "formTable": the derived forms (أوزان) of this root. For each, give the
       Roman numeral, the vowelled Arabic, a transliteration, the English
       meaning, whether that form actually OCCURS IN THE QURAN, and a short
       note on what the pattern does to the meaning. Cover Forms I-X, marking
       any that don't exist for this root with "—". Be honest about which are
       Quranic: most roots use only three or four forms in the text.

    3. "principle": the one rule worth memorising from today's root, stated as
       a rule the learner can apply to OTHER roots. E.g. "Form II on an
       intransitive verb makes it causative; on a transitive verb it makes it
       intensive." This is the transferable part of the lesson.

    4. "derivation": ONE worked derivation showing mechanically how a pattern
       and this root combine — "steps" as 2-4 short stages
       (e.g. "ق و ل + فَعَلَ → قَوَلَ", "قَوَلَ → قَالَ"), "rule" naming the
       process in Arabic and English, and "why" explaining in one or two
       sentences why Arabic does this. If the root is regular and nothing
       interesting happens, give the plain pattern-fitting steps instead.
`}
    ${isParticles
      ? '5. "derivedWords": each function word with its meaning and use.'
      : `5. "derivedWords": the 4-5 highest-frequency words IN THE QURAN built
       from this root, each with its wazn name and one line on how the pattern
       shapes the meaning.`}

    6. 2-3 REAL Quranic verses (or fragments under ~15 words) that prominently
       use today's ${isParticles ? 'words' : 'root family'} — prefer famous,
       frequently recited ones. Quote them EXACTLY with the surah:verse
       reference, transliteration, one-line English translation, and a per-word
       gloss list.

    7. 2 comprehension questions in English about what those verses say,
       answerable from the glosses.

    8. "summary": 2-3 lines closing the lesson — the shape of what was learned,
       not a restatement of every detail.

    Return ONLY valid JSON, no markdown fences:
    {
      "rootMeaning": "${isParticles ? '' : 'what the three letters share'}",
      "formTable": [
        { "form": "I", "arabic": "فَعَلَ", "romanization": "faʿala", "meaning": "English",
          "inQuran": true, "note": "what this pattern does here" }
      ],
      "waznExplanation": "${isParticles ? 'one line: these are function words — the connective tissue of every verse' : 'two sentences on the pattern(s) at work today'}",
      "principle": "the transferable rule, in one or two sentences",
      "derivation": { "steps": ["stage", "stage"], "rule": "الإعلال (weak-letter substitution)", "why": "why Arabic does this" },
      "derivedWords": [
        { "word": "الكلمة", "romanization": "transliteration", "meaning": "English meaning", "pattern": "${isParticles ? 'particle' : 'wazn name, e.g. faʿīl'}", "note": "how the pattern shapes it" }
      ],
      "verses": [
      ...
      "checkpoints": [{ "question": "English comprehension question" }],
      "summary": "how today's pieces fit together"
    }
  `;
  const result = await queryGemini(prompt, true, null, 'quick');
  ...
  const formTable = (Array.isArray(result.formTable) ? result.formTable : [])
    .filter(f => f.form && (f.arabic || f.meaning))
    .slice(0, 10)
    .map(f => ({
      form: ROMAN.includes(String(f.form).toUpperCase()) ? String(f.form).toUpperCase() : String(f.form),
      arabic: f.arabic || '—',
      romanization: f.romanization || '',
      meaning: f.meaning || '—',
      inQuran: f.inQuran === true,
      note: f.note || ''
    }));

  const derivation = result.derivation && Array.isArray(result.derivation.steps) && result.derivation.steps.length
    ? { steps: result.derivation.steps.slice(0, 5).map(String),
        rule: result.derivation.rule || '',
        why: result.derivation.why || '' }
    : null;
